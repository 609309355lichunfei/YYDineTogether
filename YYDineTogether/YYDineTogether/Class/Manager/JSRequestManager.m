...
- (void)loginWithUserName:(NSString *)userName
    ...
    [PPNetworkHelper POST:URL_Login parameters:dic success:^(id responseObject) {
        ...
        [PPNetworkHelper setValue:token forHTTPHeaderField:@"X-CSRFToken"];
...
- (void)homepageShopWithPage:(NSString *)page
    ...
    if (_token != nil && _token.length > 0 ) {
        [PPNetworkHelper setValue:_token forHTTPHeaderField:@"X-CSRFToken"];
    } else {
...
- (void)postOrderWithString:(NSString *)string
    ...
    [PPNetworkHelper POST:URL_Order parameters:string success:^(id responseObject) {
        success(responseObject);
    } failure:^(NSError *error) {
        failed(error);
    }];
