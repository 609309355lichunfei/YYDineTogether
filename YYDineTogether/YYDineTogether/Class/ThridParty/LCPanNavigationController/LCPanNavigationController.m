...
@property (strong, nonatomic) UIImageView *previousTabBarMirrorView;
...
- (void)viewDidLoad
{
    ...
    _previousTabBarMirrorView = [[UIImageView alloc] initWithFrame:self.tabBarController.tabBar.bounds];
    _previousTabBarMirrorView.backgroundColor = [UIColor clearColor];
...
- (void)setViewControllers:(NSArray *)viewControllers
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        for (NSDictionary *dic in self.shotStack) {
            NSString *name = dic[SnapshotNameKey];
            BOOL exist = NO;
        ...
        [self.shotStack removeObjectsInArray:tmpArray];
    });
}
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    ...
    if (previousViewController) {
        ...
        UIImage *tabBarImage = [self tabBarSnapshot];
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            ...
            if (tabBarImage) {
                shotInfo[tabBarImageKey] = tabBarImage;
            }
            shotInfo[contentFrameKey] = [NSValue valueWithCGRect:previousViewController.view.frame];
            shotInfo[SnapshotNameKey] = [NSString stringWithUTF8String:object_getClassName(previousViewController)];
...
- (NSArray *)popToViewController:(UIViewController *)viewController animated:(BOOL)animated;
{
    ...
    NSString *currentName = [NSString stringWithUTF8String:object_getClassName(viewController)];
    int i = 0;
    for (; i < [self.shotStack count]; i++) {
        NSString *name = [self.shotStack objectAtIndex:i][SnapshotNameKey];
        if ([currentName isEqualToString:name]) {
            break;
        }
    }
    rang = NSMakeRange(i + 1, [self.shotStack count] - i - 1);
    [self.shotStack removeObjectsInRange:rang];
...
- (void)pan:(UIPanGestureRecognizer *)pan
{
    ...
    switch (state) {
        case UIGestureRecognizerStateBegan:{
            ...
            UIImage *tabBarImage = shotInfo[tabBarImageKey];
            ...
            self.previousTabBarMirrorView.image = tabBarImage;
            CGFloat x = 0;
            CGFloat y = rect.size.height;
            CGFloat w = self.tabBarController.tabBar.bounds.size.width;
            CGFloat h = self.tabBarController.tabBar.bounds.size.height;
            CGRect tabBarRect = CGRectMake(x, y, w, h);
            self.previousTabBarMirrorView.frame = tabBarRect;//self.tabBarController.tabBar.bounds;
            self.previousTabBarMirrorView.alpha = 1;
            [self.previousMirrorView addSubview:self.previousTabBarMirrorView];
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            CGFloat k = translationPoint.x / 240;

            [self barTransitionWithAlpha:1 - k];
            if (k > 0.5) {
                self.previousBarMirrorView.alpha = k - 0.5;
//                self.previousTabBarMirrorView.alpha = k - 0.5;
            }
...
//                self.previousTabBarMirrorView.alpha = 1 - alpha;
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    if (touchPoint.x < 0 || touchPoint.y < 64 || touchPoint.x > 320) return NO;
...
- (UIImage *)tabBarSnapshot
{
    self.barBackgroundView.hidden = YES;
    UIImage *viewImage = [self.tabBarController.tabBar snapshot];
    self.barBackgroundView.hidden = NO;
    return viewImage;
}
...
- (BOOL)enablePanBack
{
    ...
    if ([self.visibleViewController respondsToSelector:@selector(enablePanBack)]) {
        UIViewController<LCPanBackDelegate> * viewController = (UIViewController<LCPanBackDelegate> *)self.visibleViewController;
...
- (void)finishPanBack
{
    if ([self.visibleViewController respondsToSelector:@selector(finishPanBack)]) {
        UIViewController<LCPanBackDelegate> * viewController = (UIViewController<LCPanBackDelegate> *)self.visibleViewController;
        [viewController finishPanBack];
    }
}
